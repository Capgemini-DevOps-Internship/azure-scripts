...
$virtualmachine1 = New-AzVM -Name agent -ResourceGroupName build-agents-powershell-westeurope -Image UbuntuLTS -Size Standard_DS1_v2 -DataDiskDeleteOption Detach -OSDiskDeleteOption Detach -VirtualNetworkName build-agents-vnet -SubnetName agents-subnet -AllocationMethod Dynamic -DomainNameLabel agentdns -Credential $Credential
...
$virtualmachine2 = New-AzVM -Name jumpbox -ResourceGroupName build-agents-powershell-westeurope -Image Win2019Datacenter -Size Standard_DS1_v2 -DataDiskDeleteOption Detach -OSDiskDeleteOption Detach -VirtualNetworkName build-agents-vnet -SubnetName jumpbox-subnet -AllocationMethod Static -PublicIpAddressName reserved-ip -Credential $Credential
